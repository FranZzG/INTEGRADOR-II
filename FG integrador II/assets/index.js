...
const renderProduct = (product) => {
  ...
  return `
    <div class="product">
    <img src=${cardImg} alt="" class= "products-img" />
...
const renderCartProduct = (cartProduct) => {
  const { id, name, cost, img, quantity } = cartProduct;
  return `
  <div class="cart-item">
    <img src=${img} alt="Productos del carrito"/>
    <div class="item-info">
      <h3 class="item-title">${name}</h3>
      <p class="item-cost">De calidad</p>
...
const getCartTotal = () => {
  return cart.reduce((acc, cur) => acc + Number(cur.cost) * cur.quantity, 0);
};
...
const createProductData = (id, name, cost, img) => {
  return { id, name, cost, img };
};
...
const addProduct = (e) => {
  ...
  const { id, name, cost, img } = e.target.dataset;
  const product = createProductData(id, name, cost, img);
...
const deleteCart = () => {
  completeCartAction(
    "Estas a punto de vaciar tu carro!,¿Quieres Continuar?",
